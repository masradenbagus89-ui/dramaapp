...
> Versi 1 · 2026-06-26 · pendamping `engine/perf-budget.mjs`
...
2. Jalankan: `npx lintasai perf-budget` (atau `node .claude-kit/engine/perf-budget.mjs`). Ubah anggaran: `--budget-kb 350`.
...
- READ-ONLY + deterministik (~0 token). Source: `engine/perf-budget.mjs`, diuji `tests/perf-budget.test.mjs`.
