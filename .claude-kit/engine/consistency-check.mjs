#!/usr/bin/env node
// engine/consistency-check.mjs - "Robot pemeriksa kecocokan" lintas-berkas, MODE PROJECT (port Node).
//
...
import {
  KIT_VERSION_CHECKS, KIT_SOURCE, KIT_TEAM_FILES_SOURCE, KIT_FACTS, KIT_RETIRED_TERMS, KIT_RETIRED_SCAN,
} from './consistency-check-data.mjs'
...
// Registry data KIT_* (versi/fakta/istilah-pensiun/cakupan-pindai) DIEKSTRAK ke
// engine/consistency-check-data.mjs (putaran-2 hemat-token: edit-fakta/istilah-pensiun = suntingan
// TERSERING di robot ini, kini cukup baca berkas data ~110 baris, bukan berkas mesin ~450 baris).
// Di-import di atas + di-RE-EXPORT di sini supaya permukaan publik consistency-check.mjs IDENTIK
// (importir lama & tes tetap bisa `import { KIT_* } from './consistency-check.mjs'`). Perilaku sama.
export { KIT_VERSION_CHECKS, KIT_SOURCE, KIT_TEAM_FILES_SOURCE, KIT_FACTS, KIT_RETIRED_TERMS, KIT_RETIRED_SCAN }
...
// MODE KIT: cek versi (4 deklarasi KIT_VERSION_CHECKS vs package.json) + buku fakta (nilai-berulang non-versi).
...
// WHY: nomor versi-saat-ini ditulis tangan di 5 tempat (package.json + 4 deklarasi KIT_VERSION_CHECKS).
