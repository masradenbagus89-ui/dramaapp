#!/usr/bin/env node
// engine/ensure-feedback-capture-hook.mjs - Nyalakan "pengingat rekam pelajaran" (engine/feedback-capture.mjs)
// dengan 1 langkah, deep-merge ke .claude/settings.json klien.
//
// KENAPA ADA: engine/feedback-capture.mjs (hook Stop yang mengingatkan AI menimbang §6.5 di akhir kerja) ikut
// tersalin ke klien (.claude-kit/engine/), TAPI sebuah hook hanya AKTIF kalau terdaftar di .claude/settings.json.
// Tanpa wiring ini, berkas hook ADA di klien tapi TAK PERNAH TERPANGGIL. Modul ini menggabungkan hook itu
// ke settings.json klien saat init/update (cermin engine/ensure-risk-gate-hook.mjs + engine/lang-hook-wiring.mjs).
//
...
//
// SIFAT (cermin engine/lang-hook-wiring.mjs / engine/ensure-risk-gate-hook.mjs - pola pemasang-hook teruji):
...
export const FEEDBACK_CAPTURE_HOOK_COMMAND = 'node "$CLAUDE_PROJECT_DIR/.claude-kit/engine/feedback-capture.mjs"'
...
// --- CLI: `node engine/ensure-feedback-capture-hook.mjs [--project-root <dir>] [--dry-run]` ---
function main() {
