#!/usr/bin/env node
// engine/project-detect.mjs - Deteksi keadaan project (versi Node). Padanan lib/project-detect.ps1.
//
...
// engine/fs-text.mjs (impor di atas) -> audit fungsi-kembar 2026-06-25. Cermin Test-Path PS + parse aman.
...
if (isMain) {
  ...
  if (root != null && (cmd === 'stack' || cmd === 'pm' || cmd === 'monorepo' || cmd === 'postsplit')) {
    ...
    console.error('Pakai: node engine/project-detect.mjs <stack|pm|monorepo|postsplit> <projectRoot>')
    process.exit(2)
  }
}
