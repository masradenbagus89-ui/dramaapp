...
//   Dispatcher (bin/lintasai.js) memetakan 'rollback' -> 'engine/rollback.mjs' (FILE INI) di COMMANDS_NODE +
...
// ---- Buang BOM awal: stripBom kini dari sumber bersama engine/fs-text.mjs (impor di atas) ----
...
// ---- Shim PowerShell deteksi reparse-point -> disatukan ke engine/reparse-guard.mjs (audit fungsi-kembar
...
// isSymlinkLike (penjaga TOCTOU lstat) -> sumber bersama engine/fs-text.mjs (impor di atas).
...
export function runRollback(argv, opts = {}) {
  ...
  if (args.projectRoot && String(args.projectRoot).trim() !== '') {
    ...
    // rollback.mjs ada di .claude-kit/engine/ -> akar project = naik 2 level (engine -> .claude-kit -> project).
