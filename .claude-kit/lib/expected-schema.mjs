#!/usr/bin/env node
// lib/expected-schema.mjs - PETA "versi-diharapkan" per-artefak milik kit (Mesin 1, STRATEGI_UPDATE_v2 §4).
//
...
//       lib/project-manifest.mjs getLintasManifestSchemaFinding (kartu project)
//       lib/migration-state.mjs (robot laporan-migrasi Mesin 2 - enumerasi SEMUA entri peta ini)
...
//       lib/project-manifest.mjs getLintasManifestStarterContentJsonc (kartu project)
//       lib/manifest.mjs saveManifest (catatan-pasang)
//       lib/migration-state.mjs recordLintasMigrationApplied (buku-besar migrasi)
//
...
// (staff-roster.yml, lintasai-portfolio.yml, consistency-map.jsonc, wiring hook) belum ber-penanda;
...
// lib/migration-state.mjs (ARTIFACT_LOCATIONS) - entri baru di sini TANPA lokasi di sana akan bikin
...
export const LINTAS_EXPECTED_SCHEMA = Object.freeze({
  ...
  // migrasi pertama dicatat lib/migration-state.mjs recordLintasMigrationApplied - tak disemai installer.
...
export function getLintasExpectedSchemaVersion(artifactKey) {
  if (!Object.prototype.hasOwnProperty.call(LINTAS_EXPECTED_SCHEMA, artifactKey)) {
    throw new Error(`Artefak '${artifactKey}' tidak terdaftar di peta versi-diharapkan (lib/expected-schema.mjs) - daftarkan dulu sebelum dipakai robot.`)
  }
