#!/usr/bin/env node
// lib/migration-state.mjs - Robot LAPORAN-MIGRASI + BUKU-BESAR migrasi artefak klien
...
//      versi-diharapkan lib/expected-schema.mjs (Mesin 1 - SUMBER TUNGGAL; robot ini enumerasi
...
export function resolveLintasArtifactPath(artifactKey, { projectRoot, kitDir }) {
  if (!Object.prototype.hasOwnProperty.call(ARTIFACT_LOCATIONS, artifactKey)) {
    throw new Error(`Artefak '${artifactKey}' terdaftar di peta versi-diharapkan tapi lokasinya belum didaftarkan di lib/migration-state.mjs (ARTIFACT_LOCATIONS) - daftarkan dulu supaya laporan migrasi tidak bolong diam-diam.`)
  }
...
export function invokeLintasMigrationCheck({ projectRoot, kitDir = null, quiet = false, expectedOverrides = {} } = {}) {
  ...
  if (!quiet) {
    ...
    console.log('Peta versi-diharapkan: lib/expected-schema.mjs milik kit yang menjalankan robot ini')
