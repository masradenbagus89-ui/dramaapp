...
// Roadmap yang disusun dari sini TETAP di-gerbang manusia (lihat workflows/7.11-peta-project.md).
//
// Cuma-baca: tidak menulis berkas, tidak menyentuh git (tak fetch/commit/config). Cetak ke stdout.
// Pakai: node lib/project-map.mjs --project-root <repo> [--since <tag|ref>] [--limit <n>] [--json] [--quiet]
...
// Jalankan git READ-ONLY dengan spawn bersih (cermin lib/repo-board.mjs). Tak pernah lewat shell.
