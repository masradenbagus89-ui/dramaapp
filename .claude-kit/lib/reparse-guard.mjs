...
// uninstall.mjs + lib/rollback.mjs (~35 baris kode keamanan). Disatukan ke 1 sumber (audit fungsi-kembar
...
//
// v2.0.0 (D4, keputusan owner): kit 100% Node -> JALUR CADANGAN PowerShell (dot-source pemeriksa PS lama)
// DIHAPUS. Deteksi reparse-point (symlink/junction) kini NODE MURNI: fs.lstatSync().isSymbolicLink()
// + ancestor-walk (cek target + tiap folder induk sampai projectRoot). Fail-secure DUA-LAPIS:
//   (1) per-path: lstat gagal -> tandai true (anggap berbahaya, cermin `catch { return $true }` lama);
...
//       daripada salah ikut junction yang menunjuk ke luar root. (Dulu ada fallback spawn pemeriksa PS;
//       kit tak lagi membawa .ps1, jadi fallback itu tak masuk akal -> ganti batal-aman.)
...
// (symlink/junction)? Cermin struktur ancestor-walk Test-PathHasReparsePoint (pemeriksa PS lama):
...
//   - lstat gagal di mana pun -> true (defensive, cermin `catch { return $true }`).
...
export function testPathsHaveReparsePoint(absPaths, projectRoot) {
  ...
  try {
    ...
    throw new Error(
      ...
      `Cek folder .claude-kit/ dari junction/symlink mencurigakan, lalu ulangi.`,
    )
  }
}
