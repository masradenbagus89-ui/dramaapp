...
6. 📐 **Cache berlapis, makin dekat user makin murah:** browser (`Cache-Control`/`ETag`) → CDN/edge (`s-maxage`, SWR) → aplikasi (Redis/memori) → query DB. Halaman publik statis → CDN + ISR/revalidate (Next.js) → `skills/nextjs/SKILL.md` · `skills/cloudflare/SKILL.md`. Data lintas-instance → **Redis/memcached** (memori-proses tak terbagi antar-server, hit-rate anjlok di multi-instance).
...
- 📐 Cache HTTP/ISR/revalidate + performa render → `skills/nextjs/SKILL.md`; cache edge/CDN → `skills/cloudflare/SKILL.md`.
