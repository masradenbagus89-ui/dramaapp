...
divisi: stack
pemicu: [vercel, railway, fly-io, render.yaml, deploy-platform]
...
> **Kapan skill ini aktif:** deteksi config — project punya `vercel.json` / `render.yaml` / `railway.json` (§4.14 auto-detect). Teks "vercel/railway/render/fly-io" jadi pemicu sekunder. Skill ini paket-stack: tarik pas-ukuran saat menggarap deployment (*deploy* = kirim versi kode ke server produksi supaya bisa diakses publik), DI ATAS standar inti (§5/§8/§11).
>
> 🙂 **Analogi:** deploy yang sehat = **toko yang buka dengan aman** — kunci disimpan di brankas platform (bukan ditinggal di meja kasir alias kode), tiap kiriman versi baru punya tombol "balik ke versi lama" kalau ternyata cacat, dan barang dagangan dicoba dulu di "panggung latihan" (preview deploy) sebelum dipajang ke pembeli asli (production).

Skill ini **advisory** (§4.17): otak native yang memutuskan adopsi/adaptasi/abaikan tiap 📐/💡. Butir **🔒 HASIL** = hasil keandalan & keamanan yang tak boleh gagal apa pun caranya. Cek fitur & harga **plan platform yang benar-benar terpasang** (Vercel/Railway/Render/Cloudflare/Supabase) — fitur berbeda per tingkatan plan dan berubah dari waktu ke waktu (§8.2 A3); jangan andalkan ingatan.
...
5. 📐 **Mitigasi Denial-of-Wallet (serangan biaya, khas serverless) — 5 langkah.** 🚨 Beda dari serangan biasa yang bikin situs TUMBANG, ini bikin **TAGIHAN MELEDAK**: serverless (Vercel/Railway) auto-scale mengikuti lalu lintas → penyerang membanjiri permintaan / memanggil fungsi mahal (LLM, ekspor, gambar) → biaya membengkak walau situs "baik-baik saja". Mitigasi:
   ...
   - 🙂 taksimu argonya jalan tiap ada penumpang — pasang batas harian + alarm biar tak "disuruh keliling kota" tanpa henti oleh iseng.
6. 📐 **Mitigasi subdomain takeover (pengambilalihan sub-domain terlantar).** Sub-domain (mis. `promo.domainmu.com`) yang lewat DNS masih menunjuk (`CNAME`) ke layanan PaaS yang **sudah kamu hapus** (Vercel/Netlify/S3) bisa **diklaim penyerang** → ia pasang situs phishing ATAS NAMA domainmu. Mitigasi: **hapus DNS record saat mematikan layanan**; audit berkala **CNAME "dangling"** (menunjuk ke tujuan kosong); jangan tinggalkan subdomain preview lama menunjuk layanan mati.
   - 🙂 jangan biarkan papan nama tokomu tetap terpasang di ruko yang sudah kamu tinggalkan — orang lain bisa buka toko palsu di situ pakai namamu.
...
8. 📐 **Gerbang keamanan otomatis di pipeline (SAST · SCA · secret · container · DAST · SBOM).** Uji kode CEPAT & OTOMATIS tiap PR — jauh lebih murah daripada ketahuan setelah bocor. Tiap alat menutup lapisan berbeda; tak perlu semua sekaligus (lihat label WAJIB/DISARANKAN/OPSIONAL):
   - **SCA — dependency rentan (WAJIB).** Cek CVE (celah publik) di paket pihak-ketiga. Kit SUDAH: `npm audit` + `govulncheck` (Go) via `npx lintasai stack-check`. TAMBAH untuk berkelanjutan: **Dependabot/Renovate** (PR otomatis saat ada versi patch) + `pip-audit` (Python). = OWASP A03:2025 → `skills/owasp/SKILL.md`.
...
9. 📐 **Keamanan infra cloud (IAM · WAF · backup/DR).** Aplikasi aman tapi "gedung"-nya bocor = sama saja bocor. 3 blok: pengaturan akun cloud, pagar depan, cadangan data — disuling untuk stack PaaS terkelola (Vercel/Railway/Render + Cloudflare + Supabase), bukan AWS mentah.
...
> **Verifikasi WAJIB cuma-baca** (§8.2 Aturan 3): membuktikan = baca config/kode + menalar, JANGAN jalankan perintah yang mengubah deployment/DB produksi saat memverifikasi.
...
- [ ] **Gerbang Pra-Rilis §4.6 LULUS** — "selesai" = terbukti (deploy jalan, healthcheck hijau, rollback teruji), bukan "sudah kutulis konfignya".
...
- 📐 **Runbook pemulihan bencana** → `docs/runbooks/`.
...
🙂 **Non-Programmer:** deploy yang matang bukan cuma "kirim kode ke server sampai bisa diakses" — ada brankas untuk kunci rahasia, tombol darurat "balik ke versi lama", panggung latihan sebelum tayang ke publik, alarm tagihan biar tak jebol mendadak, dan papan nama (subdomain) yang dicabut rapi saat toko cabang ditutup supaya tak dipakai orang lain buka toko palsu. Semua itu dicek OTOMATIS di "ban berjalan" (pipeline) sebelum kode benar-benar sampai ke pelanggan.
