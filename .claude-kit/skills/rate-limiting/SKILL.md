...
divisi: keamanan
pemicu: [rate-limit, rate-limiting, throttle, throttling, brute-force, too-many-requests, batasi-laju, anti-spam, flood-request]
rawan_keamanan: true
menggantikan: []
---
...
> **Kapan skill ini aktif:** prompt menyentuh "batasi laju / rate limit / throttle / anti brute-force / anti-spam / terlalu banyak permintaan / lindungi login-OTP-tarik-dana dari serangan bertubi". Dispatcher `rak-pemicu` menyalakannya otomatis.
>
> 🙂 **Analogi:** rate limit = **satpam pintu masuk dengan penghitung**. Tiap tamu (permintaan) boleh masuk, tapi kalau satu orang bolak-balik terlalu sering dalam semenit, satpam menahannya sebentar ("silakan coba lagi 30 detik lagi") — bukan mengusir selamanya. Tujuannya: pintu tetap terbuka untuk tamu wajar, tapi orang yang menggedor-gedor (menebak password / spam) tak bisa menjebol.

Skill ini **advisory** (§4.17): otak native memutuskan adopsi/adaptasi/abaikan tiap 📐/💡. Butir **🔒 HASIL** = jaminan keamanan yang tak boleh gagal. Cek dokumentasi penyimpanan/gateway **versi terpasang** sebelum menulis kode (§8.2 A3) — perintah atomik (Lua/`INCR`) & header rate-limit beda antar-sistem/versi. Rate limiting = **satu lapis** (mengurangi abuse), BUKAN pengganti auth kuat / anti-DDoS di edge.
...
1. 🔒 **HASIL — Penghitung di penyimpanan TERBAGI, BUKAN memori proses.** Variabel/`Map` di dalam aplikasi **tak terbagi** antar-replica (tiap instance punya hitungan sendiri → batas efektif ×jumlah-instance) dan **hilang tiap deploy** — bocor senyap di serverless/multi-instance (kelas bug sama seperti antrean in-memory, `skills/background-job/SKILL.md`). Pilih:
   ...
   - 📐 CARA BAKU: **Edge platform** (Cloudflare Rate Limiting/Durable Object, Vercel) untuk lapis terluar → `skills/cloudflare/SKILL.md`.
...
2. 🔒 **HASIL — Kunci penghitung pakai IDENTITAS yang tepat, bukan IP saja.** IP saja rapuh: banyak user di belakang satu NAT/kantor berbagi IP (kena getah bersama), penyerang gampang ganti IP. Pilih kunci per-aksi:
   ...
   - ⚠️ **Ambil IP asli dengan benar** di belakang proxy/CDN: pakai header proxy tepercaya yang di-set infra-mu (mis. `CF-Connecting-IP`), JANGAN percaya `X-Forwarded-For` mentah dari client (bisa dipalsukan) — validasi di boundary (§8 kernel).
...
4. 🔒 **HASIL — Balas 429 + `Retry-After` + header kuota.** Client (dan browser/SDK) butuh tahu **kapan** boleh coba lagi. Sertakan `Retry-After: <detik>` dan idealnya `RateLimit-Limit`/`RateLimit-Remaining`/`RateLimit-Reset` (draft IETF) supaya client sopan mundur, bukan menggedor makin keras.
...
- 💡 SARAN: butuh token-bucket / burst terkendali / limit terdistribusi matang → pakai pustaka teruji (mis. `@upstash/ratelimit`, `express-rate-limit` + store Redis, `slowapi` untuk FastAPI) ketimbang menulis sendiri. Cek API **versi terpasang** (§8.2 A3).
...
- 📐 Lapis edge (Cloudflare) → `skills/cloudflare/SKILL.md`; penghitung DB `FOR UPDATE`/antrean → `skills/supabase-prisma/SKILL.md` · `skills/background-job/SKILL.md`.
