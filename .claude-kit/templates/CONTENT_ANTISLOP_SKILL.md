...
Penempatannya sengaja **skill kustom §4.9 (opt-in), BUKAN bagian standar inti maupun perpustakaan rujukan** (§4.13). Alasannya: rujukan SEO §4.13 sudah menegaskan *"SEO strategi/off-page = di LUAR baseline teknis → kalau client punya keahlian SEO, bungkus jadi skill kustom §4.9."* Menaruh ini di aturan yang dibaca AI tiap sesi (always-load) = boros token untuk client yang tak menulis konten. Jadi: **nyala hanya kalau kamu mengadopsinya.**
...
1. **Rujuk dari `docs/SKILLS_LOCAL.md` project kamu** (berkas skill lokal milikmu; AI bikin otomatis saat kamu bikin skill pertama). Tambahkan entri:
   ...
   - Instruksi: ikuti templates/CONTENT_ANTISLOP_SKILL.md (kit) — 5 komponen di bawah.
...
- Detail vetting & penempatan: catatan serapan ECC di riwayat git repo kit (tak ikut terpasang di project client).
