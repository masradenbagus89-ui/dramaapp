#!/usr/bin/env node
// engine/update-lock.mjs - kunci "cuma boleh ada 1 update jalan" per project.
//
...
// keduanya berebut folder yang sama: proses A menyisihkan .lintasai jadi cadangan-A, proses B lalu
...
// (yang utama = siapkan-periksa-tukar di engine/kit-staging.mjs, yang aman walau tanpa kunci).
import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'
import { NAMA_FOLDER_KIT } from './project-root.mjs'

export const LOCK_DIR_NAME = `${NAMA_FOLDER_KIT}.lock`
...
export function pesanKunciDitolak(hasil) {
  ...
  baris.push(`     hapus folder penanda ini lalu ulangi: ${hasil.lockDir ?? LOCK_DIR_NAME}`)
