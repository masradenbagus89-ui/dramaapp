...
divisi: database
pemicu: [migrasi, migration, skema, schema, tabel-baru, kolom-baru, rls, database, basis-data, index-database, constraint, simpan-data, data-pelanggan, data-pengguna, tabel-data, multi-cabang, kepisah-per, simpan-permanen, jangan-hilang, rekam-medis, data-pasien, data-kesehatan, data-nasabah, postgres, postgresql, data-loss]
rawan_keamanan: true
...
🧪 **RLS multi-penyewa: pagar di DB, bukan janji di kode (§2 butir 3)**:

❌ **SALAH** (filter tenant cuma di kode aplikasi):
```ts
// "pagar" satu-satunya = WHERE di kode — endpoint lain yang lupa filter ini
// tetap dilayani DB apa adanya → data tenant lain ikut terkirim, TANPA error apa pun
const pesanan = await db.pesanan.findMany({ where: { tenantId } })
```
✅ **BENAR** (DB sendiri yang menolak baris tenant lain):
```sql
ALTER TABLE pesanan ENABLE ROW LEVEL SECURITY;
ALTER TABLE pesanan FORCE ROW LEVEL SECURITY; -- berlaku juga untuk pemilik tabel
CREATE POLICY isolasi_tenant ON pesanan
  USING (tenant_id = current_setting('app.tenant_id')::uuid);
-- cara isi identitas BEDA per stack (Supabase: auth.uid()/auth.jwt()) — cek docs provider terpasang
-- uji: set tenant A lalu SELECT baris tenant B → 0 baris walau query TANPA WHERE
```

🧪 **Migrasi destruktif tanpa snapshot (§1 + §2 butir 2)**:

❌ **SALAH** (drop / `NOT NULL` baru langsung di produksi):
```sql
ALTER TABLE pelanggan DROP COLUMN telepon;            -- data lenyap PERMANEN, tak ada jalan balik
ALTER TABLE pesanan ALTER COLUMN kupon SET NOT NULL;  -- baris lama masih NULL → migrasi meledak separuh jalan
```
✅ **BENAR** (snapshot dulu + bertahap + rollback tertulis):
```sql
-- 0) snapshot/backup DULU (pg_dump / panel provider) — syarat mutlak sebelum langkah destruktif
ALTER TABLE pesanan ADD CONSTRAINT kupon_wajib CHECK (kupon IS NOT NULL) NOT VALID; -- baris BARU dicek, lama belum → tak meledak
UPDATE pesanan SET kupon = '' WHERE kupon IS NULL;    -- backfill (bertahap kalau tabel besar)
ALTER TABLE pesanan VALIDATE CONSTRAINT kupon_wajib;  -- validasi tanpa mengunci tulis
-- rollback tertulis: ALTER TABLE pesanan DROP CONSTRAINT kupon_wajib;
-- DROP kolom: ikuti urutan §3 di atas — hapus HANYA setelah nol pemakai + snapshot
```

🧪 **Constraint di level DB, bukan cuma cek di aplikasi (§2 butir 1)**:

❌ **SALAH** (cek-dulu-baru-insert di kode = race condition):
```ts
const ada = await db.user.findFirst({ where: { email } })
if (!ada) await db.user.create({ data: { email } })
// dua request berbarengan: dua-duanya lihat "belum ada" → dua akun email sama tetap tercipta
```
✅ **BENAR** (DB benteng terakhir — duplikat ditolak atomik):
```sql
ALTER TABLE "user" ADD CONSTRAINT user_email_unik UNIQUE (email);
```
```ts
try { return await db.user.create({ data: { email } }) }
catch (e) {
  if (e.code === 'P2002') return kirim409('email sudah terdaftar') // Prisma: P2002 = UNIQUE dilanggar
  throw e // error lain jangan ditelan
}
```
