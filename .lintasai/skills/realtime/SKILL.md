...
🧪 **Otorisasi per-kanal di server saat subscribe (§2 butir 2)** — server verifikasi sesi + keanggotaan, bukan percaya klaim klien:

❌ **SALAH** (server langsung join kanal yang diminta klien = siapa pun bisa nguping):
```ts
io.on('connection', (socket) => {
  socket.on('subscribe', (kanal) => {
    socket.join(kanal) // 'chat:42' milik orang lain pun diterima → nguping kanal privat
  })
})
```
✅ **BENAR** (verifikasi identitas + keanggotaan kanal SEBELUM join):
```ts
io.on('connection', async (socket) => {
  const sesi = await verifikasiSesi(socket.handshake.auth.token) // identitas dari server, bukan klaim klien
  if (!sesi) return socket.disconnect(true)
  socket.on('subscribe', async (kanal: string) => {
    const anggota = await db.anggotaKanal.findFirst({ where: { kanalId: kanal, userId: sesi.userId } })
    if (!anggota) return socket.emit('err', 'forbidden') // bukan anggota → tolak, JANGAN join
    socket.join(kanal)
  })
})
```

🧪 **Anti-CSWSH: validasi `Origin` saat handshake WS DIY (§2 butir 2 ⚠️)** — handshake WebSocket tak tunduk same-origin:

❌ **SALAH** (auth hanya dari cookie, `Origin` tak dicek):
```ts
const wss = new WebSocketServer({ server })
wss.on('connection', (ws, req) => {
  // situs jahat bisa buka WS ke server ini — browser korban TETAP mengirim cookie (CSWSH)
  const sesi = sesiDariCookie(req.headers.cookie)
  if (sesi) daftarkan(ws, sesi)
})
```
✅ **BENAR** (whitelist `Origin` + token koneksi, bukan cookie otomatis):
```ts
const BOLEH = new Set(['https://app-kamu.com']) // whitelist domain sendiri
const wss = new WebSocketServer({
  server, // API paket `ws` — cek docs versi terpasang (alternatif: tangani event 'upgrade' manual)
  verifyClient: async ({ origin, req }, cb) => {
    if (!BOLEH.has(origin)) return cb(false, 403) // Origin asing → handshake ditolak sebelum koneksi berdiri
    cb(!!(await verifikasiSesi(tokenDari(req.url))), 401) // token DIVERIFIKASI, bukan cuma dicek ada
  },
})
```

🧪 **Filter payload per-penerima — jangan siarkan baris DB mentah (§2 butir 3 + §1 Rahasia)**:

❌ **SALAH** (objek DB langsung di-broadcast ke semua pelanggan):
```ts
const pesan = await db.pesan.create({ data: masuk, include: { pengirim: true } })
io.to(kanal).emit('pesan', pesan) // kolom internal ikut tersiar: pengirim.email, pengirim.passwordHash
```
✅ **BENAR** (payload dibentuk eksplisit — pilih field satu-satu):
```ts
const pesan = await db.pesan.create({ data: masuk, include: { pengirim: true } })
io.to(kanal).emit('pesan', {
  id: pesan.id, isi: pesan.isi, dikirimPada: pesan.dikirimPada,
  pengirim: { id: pesan.pengirim.id, nama: pesan.pengirim.nama }, // JANGAN spread objek DB — kolom baru otomatis ikut bocor
})
```
