...
> 🙂 **Inti:** data pribadi pelanggan bukan milikmu untuk dipakai sesuka hati — kamu cuma boleh memakainya untuk keperluan yang sudah diizinkan, wajib menyimpannya aman, menghapusnya kalau diminta, dan wajib lapor kalau terjadi kebocoran.
...
- **Rotasi kunci** terjadwal; kunci enkripsi JANGAN di repo/`.env` yang ikut ter-commit (§5.2).
- **Jangan log PII/secret mentah** (§5.2) — log yang bocor = kebocoran data. Redaksi (samarkan) email/nomor di log.
- **Batasi akses** — RLS default-deny (`skills/database/SKILL.md`), pisah data sensitif, kolom rahasia jangan ikut ke API/`SELECT *` (rujuk `skills/python/SKILL.md` DRF `__all__`).
...
- Retensi & hapus data (mekanik): kebijakan retensi + hapus terjadwal (lifecycle storage + purge DB) → `templates/SAFE_DATABASE_OPERATIONS.md`.
- Respon + notifikasi kebocoran → `templates/SECURITY_INCIDENT_PLAYBOOK.md`.
- Operasi DB aman (kolom sensitif, migrasi) → `templates/SAFE_DATABASE_OPERATIONS.md`; aturan siapa-boleh-lihat-baris-mana (RLS) → `skills/database/SKILL.md`.
...
- **Dependensi:** `SECURITY_INCIDENT_PLAYBOOK.md`, `SAFE_DATABASE_OPERATIONS.md`, `skills/analytics/SKILL.md`.
