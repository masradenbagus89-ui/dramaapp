...
> Pelengkap, BUKAN pengganti: Row-Level Security = `skills/database/SKILL.md` · backup harian otomatis = fitur backup/PITR bawaan Supabase (Dashboard → Database → Backups) · akses berjenjang junior/senior (DB role tiering) = minta AI susun SQL `GRANT`/`REVOKE` per schema (`skills/database/SKILL.md`). File ini fokus ke **2 hal yang belum dicakup**: (1) ubah struktur tanpa-downtime (*expand-then-contract*), (2) cara balik kalau migrasi gagal (*rollback runbook*).
