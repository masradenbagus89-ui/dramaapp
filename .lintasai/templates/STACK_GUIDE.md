...
> ⚠️ **PENTING — panduan ini KHUSUS stack Next.js + Vercel + Supabase**, bukan "universal lintas-stack". Kalau proyekmu pakai stack lain (Python/Django, Go, Rust, PHP/Laravel, dll.), JANGAN paksakan panduan ini — minta AI buatkan panduan sesuai stack-mu (AI deteksi stack dari `package.json` + berkas penanda lalu menyesuaikan). Yang "universal" di kit ini adalah **aturan kerja** (`AGENTS.md` + rak `skills/`), bukan pilihan stack.

File ini = **panduan opinionated** untuk stack standar tim AI-first.
Target stack default:

- **Frontend + Backend** → Next.js (App Router) + TypeScript — angka versinya di `templates/STACK_VERSIONS.md` (acuan tunggal; jangan salin angka ke sini biar tak beda-beda)
...
```text
app/
├── global-error.tsx      // jaring TERAKHIR: error di ROOT layout (error.tsx tak menangkapnya)
├── dashboard/
...
⚠️ **Batas `error.tsx` yang sering bikin layar putih:** ia TIDAK menangkap error yang terjadi di
`layout.tsx` pada segment yang SAMA (boundary-nya berada di dalam layout itu). Error di **root layout**
hanya tertangkap `app/global-error.tsx` — dan berkas itu menggantikan seluruh dokumen, jadi ia WAJIB
me-render `<html>` dan `<body>`-nya sendiri. Tanpa `global-error.tsx`, root layout yang gagal =
**layar putih total tanpa pesan apa pun**.
...
export default function Error(
  ...
  // JANGAN tampilkan `error.message` mentah: isinya bisa memuat pesan SQL / path folder / nama kolom
  // internal. Pesan asli tetap utuh di log server + Sentry; yang ke layar = kalimat awam + `digest`
  // (kode kejadian) supaya user bisa menyebutkannya ke support dan kita cocokkan di log.
  return (
    <div>
      <h2>Ada gangguan sesaat</h2>
      <p>Coba lagi sebentar lagi. Kalau masih gagal, sebutkan kode ini ke kami.</p>
      {error.digest && <p>Kode: {error.digest}</p>}
      <button onClick={reset}>Coba lagi</button>   {/* WAJIB ada: tanpa reset user terkunci di layar ini */}
    </div>
  )
}
```
...
> ⚠️ **Sadar-versi (Next.js 16+):** sejak Next.js 16, berkas ini **berganti nama jadi `proxy.ts`** (fungsi ekspor juga `proxy`, bukan `middleware`) dan jalan di **runtime Node.js**. `middleware.ts` masih bisa dipakai untuk Edge tetapi **sudah usang (deprecated)** dan akan dihapus. Ada codemod resmi untuk migrasi. Larangan salah-koreksi + detail ada di `skills/next-core/SKILL.md`. **Cek angka `next` di `package.json` sebelum menyentuh berkas ini** (§2.1) — contoh di bawah pakai gaya pra-16.
...
Index = struktur tambahan yang mempercepat pencarian baris tanpa membaca semua data di tabel. Salah pilih tipe/urutan = query tetap lambat walau kolomnya sudah di-index.
...
- 🙂 Non-Programmer: index mempercepat pencarian data. **Urutan** kolom penting — "nama depan lalu tanggal" beda hasil dari "tanggal lalu nama depan". **Partial** = index yang cuma mencakup baris yang masih aktif (lebih ringkas & cepat). **Covering** = index yang sudah menyimpan data yang dibutuhkan, jadi tak perlu buka tabel aslinya lagi.
...
```
> Sadar-versi: sejak Postgres 15, hak `CREATE` di schema `public` sudah dicabut dari PUBLIC secara bawaan — perintah di atas aman dijalankan tapi jangan diklaim "perbaikan" kalau sudah PG15+. Query #2 butuh `CREATE EXTENSION IF NOT EXISTS pg_stat_statements;` dulu (di Supabase umumnya sudah aktif — cek via `list_extensions`). Kontrol-akses REVOKE per-developer yang lebih dalam: minta AI susun SQL `GRANT`/`REVOKE` per schema (role tiering; skills/database).

- 🙂 Non-Programmer: tiga query pertama = cek kesehatan database — cari foreign key yang belum ter-index (pencarian lambat), query paling lambat, dan baris data lama yang belum dibersihkan. Perintah terakhir = kunci akses default supaya tak sembarang akun bisa membuat objek baru di database.
...
- 🙂 Non-Programmer: pilih tipe data yang pas. Uang JANGAN pakai tipe `float` — nilainya tak presisi, selisih kecil menumpuk jadi total yang salah. Waktu pakai `timestamptz` supaya jam tak kacau saat pengguna beda zona waktu.

> **Catatan konsistensi kit:** baris "Waktu → `timestamptz`" MENGUATKAN aturan universal DB (skills/database: `kolom waktu _at`, timezone-aware) — bukan aturan baru yang bentrok. Untuk **ID**, contoh Prisma kit memakai `@default(cuid())` (string) — itu sah untuk skala tim kecil; baris "ID → `bigint`" adalah trade-off performa index, BUKAN keharusan. Diskusikan dulu (§1.1) sebelum mengganti konvensi ID yang sudah jalan.

### 5.4. Pengaman timeout query & transaksi (guardrails)

> Guardrail = batas otomatis di DB supaya satu query/transaksi nyasar tak menyandera koneksi + mengunci tabel tanpa batas. BEDA dari index (kecepatan) — ini soal MEMBATASI KERUSAKAN saat ada yang salah.
...
- 🙂 Non-Programmer: kalau satu proses butuh lebih dari 30 detik, otomatis dihentikan supaya proses lain tak ikut macet.
...
> 🙂 Awam: server DB kita disewa dan dikelola pihak Supabase — kita tak boleh mengubah setelan level sistem (`ALTER SYSTEM`); ubah setelan lewat per-role/per-db atau Dashboard saja.
...
- [ ] `NEXT_PUBLIC_*` cuma untuk data yang aman ke browser (mis. URL publik) - JANGAN API key, JANGAN service_role_key.
...
- [ ] Pakai `anon_key` (limited) di browser, `service_role_key` di Server Action saja. **(Berlaku untuk arsitektur NON-split / monolith. Pada split-repo / microservice: frontend NOL akses DB — JANGAN colok Supabase dari browser sama sekali; semua data lewat API backend.)**
- [ ] Row-Level Security (RLS) aktif di semua tabel publik.

### 7.4. Security Headers + CSP (Content-Security-Policy)

> **CSP** = aturan yang dikirim ke browser: "skrip/gambar/font hanya boleh dimuat dari sumber yang kusebut". Ini penahan utama **XSS** (penyusupan skrip jahat ke halaman).
...
- Dipasang lewat berkas yang menyaring tiap request masuk — 🚨 **SADAR-VERSI**: `proxy.ts` (fungsi `proxy`) di Next 16+, `middleware.ts` di versi sebelumnya. Lihat peringatan di `skills/next-core/SKILL.md` + contoh UTUH di dok resmi: `nextjs.org/docs/app/guides/content-security-policy` (jangan tulis dari ingatan — header `x-nonce`, `'strict-dynamic'`, dan matcher pengecualian `_next/static` semuanya ada di sana).
...
Tim tanpa peran QA → satu celah keamanan/aksesibilitas (a11y) yang lolos mata bisa terlanjur tayang. ESLint bisa otomatis menangkap pola bahaya secara pasti (deterministik) saat ngoding — TAPI dipasang **bertahap** supaya tak membanjiri merah (banjir peringatan wajar = staff mematikannya diam-diam, malah lebih buruk dari tak ada gerbang).
...
> Kenapa dipisah `error` vs `warn`: rule keamanan (XSS) hampir tak pernah salah-tangkap → aman jadi `error`. Rule a11y/selera lebih sering "peringatan wajar" → `warn` dulu supaya alur kerja tak macet. Naikkan `warn`→`error` per-rule saat timmu siap.

---

## 8. Decision Matrix: Vercel vs Railway vs Render
