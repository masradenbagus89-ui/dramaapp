...
**Terakhir diisi:** 2026-09-14 (malam) — **WEBHOOK PLAYLY SELESAI DIBANGUN & TERBUKTI JALAN,
tapi BELUM di-commit, BELUM tayang, dan BELUM bisa menyala** (menunggu sisi Playly).
Branch **`feat/playly-webhook`** (baru, dari `origin/main`). Rinciannya di bagian paling bawah.
...
## 🧭 2026-09-14 (malam) — Webhook Playly: dibangun, diuji, BELUM di-commit

**Branch `feat/playly-webhook`** (dicabang dari `origin/main` hari ini). **Belum di-commit,
belum di-push, belum tayang.** Owner belum meminta commit — kalau sesi ini ditutup tanpa
commit, pekerjaannya ada di working directory, bukan di git.

**Apa ini:** `POST /api/webhooks/playly` — pintu masuk supaya Playly bisa MENDORONG
"ada video baru" ke DramaKu seketika, menggantikan tarikan berkala yang sekarang jalan.

### ⚠️ Tiga hal yang harus dibaca sebelum melanjutkan

1. **Endpoint ini TIDAK akan pernah dipanggil sampai pengelola Playly menyalakannya.**
   Nol bukti Playly punya fitur kirim webhook (nol jejak di seluruh repo). Nama header,
   nama event, dan bentuk payload semuanya dari spesifikasi owner. **Langkah owner:
   hubungi pengelola Playly** — tanyakan apakah webhook tersedia, lalu minta nama header
   & bentuk payload yang sebenarnya.
2. **Belum menggantikan polling.** Data webhook tersimpan tapi **belum tersambung** ke
   `/playly` maupun `/discover`. Menyambungkannya mengubah `getPlaylyVideosPublik()` yang
   dipakai 3 pemanggil — perubahan cakupan, butuh keputusan owner (§4.4). Sudah tersedia
   `getPublishedPlaylyWebhookVideos()` untuk dipakai saat itu diputuskan.
3. **Anti-replay belum tertutup.** Payload tak membawa `event_id`/`sent_at`, jadi
   permintaan sah yang pernah terekam bisa dikirim ulang orang lain dan tetap lolos.

### Env baru yang WAJIB dipasang di Vercel

`PLAYLY_WEBHOOK_SECRET` — nilai acak panjang, **sama persis** dengan yang dipegang Playly.
Buat dengan: `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`.
Kosong = endpoint balas **503** dan tidak memproses apa pun (disengaja). Dokumentasi
lengkapnya di `.env.example`.

**Dua cara pembuktian diterima** (keputusan owner lewat popup, sesi ini) — Playly pakai
yang mereka dukung, secret-nya sama:

| Cara | Header | Catatan |
|---|---|---|
| A | `x-playly-secret: <secret>` | Paling sederhana. Pola yang sama dipakai `/api/agent/video-base` |
| B | `X-Playly-Signature: <HMAC-SHA256 hex>` | Lebih aman — secret tak lewat kabel, isi pesan ikut terkunci. `sha256=<hex>` juga diterima |

### Keputusan owner yang membatalkan permintaan awalnya sendiri

Permintaan awal sesi ini: endpoint di `app/api/playly/video-baru/route.ts`, verifikasi
hanya `x-playly-secret`, dikerjakan di `feat/playly-integrasi`. Sesudah disajikan
temuannya, owner memilih: **alamat tetap `/api/webhooks/playly`** · **dua cara verifikasi
diterima** · **branch baru dari `origin/main`**. Jangan "perbaiki" ini balik ke permintaan
awal di sesi berikutnya — itu keputusan sadar, bukan kelalaian.

### 🔴 Pelajaran mahal — dua berkas di folder kerja ternyata sudah bergerak di `origin/main`

Folder kerja memuat perubahan paginasi `lib/playly.ts` + `tests/playly-paginasi.test.ts`
yang **sudah di-commit ke `origin/main` sebagai `852c989`** beberapa jam sebelumnya, dan
`HANDOFF.md` versi lokal yang **tertinggal 9 commit**. Memindahkan semuanya utuh ke branch
baru = commit duplikat + 9 catatan rekan terhapus diam-diam.

**Aturan yang lahir dari sini:** sebelum memindahkan kerja yang belum di-commit ke branch
lain, jalankan `git log <branch>..origin/main -- <berkas>` untuk **tiap** berkas. Hitungan
0 = aman dipindah utuh; >0 = berkasnya sudah bergerak, jangan ditimpa.

Cadangan pekerjaan lama ada di `git stash` (`stash@{0}`, "wip webhook playly sesi
2026-09-14") kalau suatu saat perlu dilihat lagi.

### Bukti (dijalankan, bukan dibaca)

- `npx tsc --noEmit` → **exit 0**
- `npx vitest run` → **44 berkas, 604 tes lulus** (82 tes webhook)
- `npx next build` → **exit 0**, route terdaftar `ƒ /api/webhooks/playly`
- **Uji HTTP langsung** (dev server port 3077, mode file): tanpa header → **401** · kunci
  salah → **401** · `x-playly-secret` benar → **200 `aksi:"dibuat"`** · kiriman ulang →
  **200 `aksi:"diperbarui"` (tetap 1 baris)** · HMAC benar → **200** · badan diubah setelah
  ditandatangani → **401** · embed domain asing → **400** · unpublish → **200 `hidden:true`**
- **Pertahanan XSS terbukti di data tersimpan:** `<script>` yang sengaja diselipkan di
  `embed_code` **tidak ikut tersimpan** — yang masuk hanya `src`-nya.
- `data/playly.json` dikembalikan ke keadaan semula sesudah uji (504 byte, nol data uji).

Rincian lengkap: `docs/lintasai/rencana/2026-09-14-webhook-playly.md`
