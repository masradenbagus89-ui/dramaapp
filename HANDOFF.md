...
**❓ BELUM terbukti tayang di situs sungguhan.** Push ≠ tayang: Vercel masih perlu membangun ulang. **Sengaja TIDAK di-polling** (pelajaran 2026-09-24: loop `curl` memicu Vercel Security Checkpoint dan memblokir komputer sendiri). Owner yang membuktikan — buka `https://dramaapp.vercel.app/playly`, **klik salah satu kartu video**, lalu di bawah pemutar harus muncul kotak putih berisi judul + tombol DOWNLOAD/Bagikan/Simpan. Dashboard Vercel → Deployments → cari `625fe95` menjawabnya dalam sedetik.
