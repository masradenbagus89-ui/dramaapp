...
**Terakhir dicek:** 2026-09-02 (sore). **Lokal = `origin/main` = `dramaku/main` = `8ff07e9`, selisih NOL ✅** (kesejajaran logo · judul hero · label film di satu garis kiri; dual push, hash dibandingkan sesudah `git fetch` ulang). **Cek kesehatan produksi:** `/` · `/beranda` · `/discover` · `/shorts` · `/playly` semua **200** · HTML landing produksi: header `flex h-16 … px-4 md:px-6` dan hero `relative flex min-h-[70svh] … px-4 md:px-6` — **nol** sisa `max-w-7xl` di keduanya, label film tetap `left-4 md:left-6` (patokan). Tayang ~60 detik sesudah push.
