...
|---|---|---|---|
| 🔴 **TERTAHAN — push produksi 403** | `0e7a5c5` | Integrasi Playly + perbaikan yang membuatnya jalan (pola `/id/{id}/embed`, alamat relatif, katalog publik saat kunci mitra ditolak) | Owner memberi izin 2026-08-25. **`origin` (ojokesusu/dramaku): SUKSES** `617f6a0..0e7a5c5`. **`dramaapp` (dipantau Vercel): DITOLAK 403** — `gh` login sebagai `yusufscorpio` yang tak punya akses tulis. Produksi masih 404 (dicek 4 menit). Sebelum push: 339 tes lulus, `tsc` exit 0, `next build` sukses (5 route Playly terdaftar), nol secret di diff, video terbukti berputar |
