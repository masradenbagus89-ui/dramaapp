...
**Terakhir dicek:** 2026-09-08 (lanjutan). **Lokal = `origin/main` = `dramaku/main` = `88fbe18`, selisih NOL** (baris FILM UNGGULAN diisi penuh: 5 -> 14 poster). Dual push `f2b91c4..88fbe18`, hash dibandingkan lewat `git ls-remote` LANGSUNG ke server. Pra-push: `tsc` 0 error, 416 tes lulus, `next build` sukses. **TAYANG & TERVERIFIKASI:** `/` `/beranda` `/discover` semua 200; halaman depan produksi kini **14 kartu poster** (sebelumnya 5, sisa lebar kanan kosong), 6 tautan genre, bar cari magenta ada, nol carousel berjalan. `/discover` SENGAJA tidak diubah & terbukti masih punya hero sinematiknya (`min-h-[80svh]` 1x, carousel 1x). Deploy Vercel ~1 menit: percobaan pertama masih 5 kartu, kedua sudah 14.
