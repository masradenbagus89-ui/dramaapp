...
**Terakhir dicek:** 2026-09-08 (malam, lebar penuh). **Lokal = `origin/main` = `dramaku/main` = `1b7729d`, selisih NOL** (baris poster melebar sampai tepi layar + poster diperkecil). Dual push `cefe5bc..1b7729d`, hash dibandingkan lewat `git ls-remote` LANGSUNG ke server. Pra-push: `tsc` 0 error, 424 tes lulus, `next build` sukses. **TAYANG & TERVERIFIKASI:** `/` `/beranda` `/discover` semua 200; CSS produksi `/_next/static/chunks/3jiqbnsc0528u.css` berisi **`.shell-wide{max-width:100%}`** (batas 1440px benar-benar lepas di sisi penonton, bukan cuma di kode); HTML memakai kelas kartu baru `w-24 sm:w-28 md:w-32` dengan NOL sisa ukuran lama; 92 kartu poster dalam 6 baris; tetap 1 h1. Deploy Vercel ~1 menit: percobaan 1 masih versi lama, percobaan 2 sudah baru. **Catatan cara verifikasi:** CSS Next.js ada di `/_next/static/chunks/*.css`, BUKAN `/_next/static/css/*.css` — pola grep lama meleset dan sempat memberi kesan "tidak ketemu".
